...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None,
    ...
    def thunk_plus():
        ...
        if kwargs['env_name'] in dict_gym:
            ...
            if exp_name == 'cmd_ppo_pyco':
                ...
                def test_pyco():
                    ...
                    #ITS HERE THAT THE PYCOLAB GAME IS CALLED! NO MAIN SO NO HUMAN_UI. To get a human UI You need to pass by a python console.
            ...
            elif exp_name == 'cmd_ppo_pyco_load':
                env_name = kwargs['env_name']
        ...
        # Run thunk
        if env_name in dict_gym:
            gym_or_pyco = 'gym'
        else:
            gym_or_pyco = 'pyco'
        thunk(gym_or_pyco, **kwargs)

        # THIS IS WITH THUNK THAT EVERYTHING IS RUNNING AS THUNK = PPO_PYCO HERE
