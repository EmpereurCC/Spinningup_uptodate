...
raw_color_txt = """aqua	#00FFFF	(0,255,255)
...
_colour_lines = raw_color_txt.split("\n")


def line_to_kv(line):
    """Convert the raw string to a key-value pair"""
    colour_name = line[:line.find("#")].strip()
    colour_rgb_values = tuple([int(v) / 255.0 for v in line[(line.find("(") + 1):line.find(")")].split(",")])
    return colour_name, colour_rgb_values


def colour_dictionary():
    """Create a dictionary of colours (name:rgb)"""
    colours = dict()
    for line in _colour_lines:
        k, v = line_to_kv(line)
        colours[k] = v
    return colours
